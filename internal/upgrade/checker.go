...
func FormatError(s *SchemaStatus) string {
	...
	if s.CurrentVersion > s.RequiredVersion {
		return fmt.Sprintf(
			"Database schema (v%d) is newer than this binary (requires v%d).\n"+
				"You may be running an older version of mtclaw.\n\n"+
				"  Fix: upgrade your mtclaw binary to the latest version.\n",
	...
	return fmt.Sprintf(
		"Database schema is outdated: current v%d, required v%d.\n\n"+
			"  Run:  ./mtclaw upgrade\n"+
			"  Or:   ./mtclaw migrate up   (SQL-only, no data hooks)\n\n"+
			"  Docker/CI: set MTCLAW_AUTO_UPGRADE=true to upgrade automatically on startup.\n",
		s.CurrentVersion, s.RequiredVersion,
	)
}
